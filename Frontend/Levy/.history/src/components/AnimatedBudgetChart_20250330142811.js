...
const AnimatedBar = ({ category, percentage, color, budget , navigation}) => {
    ...
    return (
        <Pressable 
        onPress={()=>{
            navigation.navigate('detailBudget',{budeget});
        }}
...
const AnimatedBudgetChart = ({ budgetList ,navigation}) => {
        ...
        <ScrollView contentContainerStyle={{
            ...
            <View className="py-[16px] gap-[24px] flex-col flex">
                {budgetData.map((item, index) => (
                    <AnimatedBar key={index} category={item.category} percentage={item.percentage} color={item.color} budget={item} navigation={navigation} />
                ))}
            </View>
        </ScrollView>
    );
