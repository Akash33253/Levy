...
export default function Footer({ navigation, color }) {
    ...
    return (
        <View className="py-[10px] flex flex-row justify-between w-full items-center px-[30px] rounded-t-[30px] bg-white">
            <View className={`absolute top-0 left-1/2 -ml-[5px] -mt-[27px] flex items-center justify-center w-[70px] h-[70px] rounded-full`}>
                <Pressable
                    onPress={() => {
            ...
            <View className="flex flex-row gap-[35px]">
                <Pressable className="flex flex-col gap-[4px] items-center">
                    ...
                    />
                    <Text className={`text-[10px] ${footerTab === 'budget' ? "text-violet-100" : "text-[#C6C6C6]"}  font-medium`}>Budget</Text>
                </Pressable>
                <Pressable className="flex flex-col gap-[4px] items-center"
                    onPress={() => {
