...
const RingWithSegments = () => {
    ...
    const generate =async ()=>{
        ...
        const value = total + await Math.random()*10
        const newTotal = Math.max(0, Math.round(value)); // Ensure positive integer
        setTotal(newTotal);
    }
    return (
        <View style={{
            ...
            <Canvas style={{
                ...
                />
                <Text text={targetText} color={'black'} y={100} x={100} font={font} />
            </Canvas>
        </View>
    );
};
