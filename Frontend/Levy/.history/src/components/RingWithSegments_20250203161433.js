...
const RingWithSegments = () => {
    ...
    const totalExpense = monthlyTotalExpense?.totalExpense || 0; // Prevent undefined issues

    if (totalExpense === 0) {
        return <View style={{ width: radius * 2, height: radius * 2, marginTop: 10 }} />;
    }
    ...
    // Precompute segment positions safely
    let runningSum = 0;
    const segmentData = monthlyCategoryExpense.map((expense) => {
        if (!expense.totalAmount || !categoryData[expense.category]) return null; // Skip invalid entries
            ...
            <Canvas style={{ flex: 1 }}>
                ...
                {segmentData.map((segment, index) => (
                    <Path
                        ...
                        strokeCap="round"
                        start={segment.start}
                        end={animatedSegments.value[index]?.end ?? segment.start} // Ensure valid value
                    />
                ))}
            </Canvas>
        </View>
