...
export default function Transaction({ navigation }) {
    ...
    const { monthlyTotalExpense, getTotalMonthlyExpense, monthlyExpenses, getMonthlyExpenses, categoryData, getExpenseDetail } = context
            ...
            >
                <View className="flex-1">
                    ...
                    <View className="w-full  px-[16px] py-[8px]">
                        <Pressable className="bg-[#EEE5FF] px-[16px] py-[8px] rounded-[8px] flex flex-row items-center justify-between">
                            <Text className="text-[16px] text-violet-100 font-normal">See your financial report</Text>
                            <Image 
                    ...
                    <View className="flex flex-col w-full items-center justify-center mt-[20px] gap-[9px]">
                        <Text className="text-light-20 text-[14px] font-medium">Total Month Expense</Text>
                        ...
                        <View className="flex flex-col]">
                            {
                                monthlyExpenses.map((expenses, index) => {
                                    return <View key={index} className="w-full flex flex-col">
                                        <Text className="text-black text-[18px] py-[13px] font-semibold">{formattedDate(expenses[0])}</Text>
                                                        ...
                                                        <View className="flex-1 flex flex-row justify-between">
                                                            <View className="flex flex-col gap-[13px]">
                                                                ...
                                                                <Text
                                                                    numberOfLines={2}
                                                                    ellipsizeMode="tail"
                                                                    className="w-[100px] text-[13px] font-medium text-light-20">
                                                                    {expense?.description}
                                                                </Text>
                                                            </View>
