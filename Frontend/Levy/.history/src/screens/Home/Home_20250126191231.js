...
export default function Home({ navigation }) {
  ...
  const { getRecentExpenses, recentExpenses, categoryColors } = context;
              ...
              >
                <View className="flex flex-col gap-[8px]">
                  {recentExpenses?.map((expense) => {
                    let categoryName = expense.category.toLowerCase();
                    return <Pressable className="px-[14px] py-[17px] bg-light-80 rounded-[24px] flex flex-row gap-[9px] w-full items-center">
                      <View
                        style={{
                          backgroundColor: categoryColors[expense?.category]
                        }}
                        className={`w-[60px] h-[60px] rounded-[16px] flex items-center jut`}>
