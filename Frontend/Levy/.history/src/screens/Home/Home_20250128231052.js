...
export default function Home({ navigation }) {
  ...
  const { getRecentExpenses, recentExpenses, categoryData,todayTotalExpense,getTotalTodayExpense} = context;
          ...
          >
            <View className="flex-1 flex flex-col gap-[8px]">
              <View className="w-full px-[16px] flex flex-row justify-between items-center">
                ...
                <Pressable className="px-[16px] py-[7px] rounded-[40px] bg-violet-20" onPress={()=>{navigation.navigate("")}}>
              ...
              >
                <View className="flex flex-col gap-[8px]">
                  {recentExpenses?.map((expense) => {
                    return <Pressable key={expense?._id} className="px-[14px] py-[17px] bg-light-80 rounded-[24px] flex flex-row gap-[9px] w-full items-center">
                      <View
                        style={{
