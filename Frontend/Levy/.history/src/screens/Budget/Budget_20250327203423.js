...
export default function Budget({ navigation }) {
    ...
    const processBudgetAndExpenses = (budget, expenses) => {
        // Convert budget array into an object for quick lookup
        const budgetMap = budget.reduce((acc, item) => {
            acc[item.category] = item.amount;
            return acc;
        }, {});

        // Filter only expenses that have a corresponding budget entry
        const mergedData = expenses
            ...
            .map(expense => ({
                ...
                budgetAmount: budgetMap[expense.category] // Guaranteed to exist
            }));
        ...
        try {
            ...
            if (token) {
                ...
                if (json && json.success) {
                    const result = processBudgetAndExpenses(json.budget, json.expenses);
        ...
        <SafeAreaView className="flex-1 bg-violet-100">
            ...
            <View className="flex-1 bg-white rounded-t-[32px]">
                <View className="flex-1">
                    <View className="flex-1">
                        {
                            budgetList.length>0?(((budget.expense / budget.budgetAmount) * 100).toFixed(2)):''
                        }
