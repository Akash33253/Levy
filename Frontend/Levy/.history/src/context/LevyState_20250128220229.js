...
const LevyState = (props) => {
    ...
    const getTotalTodayExpense = async () => {
        try {
            ...
            if (token) {
                ...
                const json = await response.json();
                if (json && json.success&&json.totalExpense) {
    ...
    const getTotalMonthlyExpense = async (params) => {
        try {
            ...
            if (token) {
                const response = await fetch(`http://${ipTemp}:5000/expense/fetchTotalMonthlyExpense?year=${year}&month=1`, {
                    method: "GET",
                    headers: {
                        "Content-Type": "application/json",
                        "auth-token": token
