...
export default function Login({ navigation }) {
    ...
    const { setIsLoading, ipTemp, setUser } = context;
    ...
    const loginUser = async () => {
        setIsLoading(true);
        try {
            const response = await fetch(`http://${ipTemp}:5000/auth/login`, {
                method: "POST",
                headers: {
            ...
            <View className="flex w-full py-[55px] px-[16px] gap-[60px] flex-col">
                <View className="flex flex-col gap-[24px]">
                    <View className="flex px-[12px] py-[16px] w-full border-[1px] border-light-60 rounded-[16px]">
                        <TextInput
                            className="w-full text-[16px] placeholder:text-light-20 font-inter"
                    ...
                    <View className="flex px-[12px] py-[16px] w-full border-[1px] border-light-60 rounded-[16px] gap-[16px] flex-row items-center">
                        <TextInput
                            className="flex-1 text-[16px] placeholder:text-light-20 font-inter"
                ...
                <View className="flex flex-col gap-[33px] items-center">
                    <Pressable className="w-full" onPress={() => { loginUser() }}>
                        <View className="w-full bg-violet-100 rounded-[16px] p-[18px]">
                            <Text className="text-center text-light-80 font-inter-semibold text-[18px]">Login</Text>
                        </View>
                    </Pressable>
                    <Pressable>
                        <Text className="text-violet-100 text-[16px] font-inter-medium">Forgot Password?</Text>
                    </Pressable>
                    <View className="flex flex-row items-center">
                        <Text className="text-[16px] text-light-20 font-inter-medium">Don't have an account? <Pressable onPress={() => { navigation.replace("signup") }}><Text className="text-violet-100 text-[16px] font-inter-medium">Sign up</Text></Pressable></Text>
                    </View>
                </View>
            </View>
        </SafeAreaView>
