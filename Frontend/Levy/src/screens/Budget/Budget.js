...
export default function Budget({ navigation }) {
    ...
    const processBudgetAndExpenses = (budget, expenses, id) => {
        ...
        try {
            ...
            if (token) {
                ...
                if (json && json.success && json.budgetId) {
                    const result = processBudgetAndExpenses(json.budget, json.expenses,json.budgetId);
        ...
        <SafeAreaView className="flex-1 bg-violet-100">
            ...
            <View className="flex-1 bg-white rounded-t-[32px]">
                <View className="flex-1">
                    <View className="flex-1">
                        {
                            budgetList.length > 0 ? <AnimatedBudgetChart navigation={navigation} budgetList={budgetList} /> :
