...
expenseRouter.get('/monthlyCategoryExpense', fetchUser, async (req, res) => {
  try {
    ...
    // Convert to integers
    const yearInt = parseInt(year);
    const monthInt = parseInt(month) - 1; // Month in JavaScript is 0-based (Jan = 0)

    // Start and end of the month
    const startDate = new Date(yearInt, monthInt, 1);
    const endDate = new Date(yearInt, monthInt + 1, 0, 23, 59, 59);
    ...
    // Format response to include all categories, even if they have no expenses
    ...
    const result = categoryList.map(category => {
      const expense = expenses.find(exp => exp._id === category);
      return { category, totalAmount: expense ? expense.totalAmount : 0 };
    });

    return res.json({ success: true, monthlyCategoryExpenses: result });
