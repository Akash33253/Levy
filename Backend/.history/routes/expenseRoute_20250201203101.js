...
expenseRouter.get('/fetchMonthlyCategoryExpense', fetchUser, async (req, res) => {
  try {
    ...
    console.log("Fetching expenses between:", startDate, "and", endDate);
    ...
    // Process category-wise totals
    const categoryExpenses = {};

    expenses.forEach(expense => {
      const category = expense.category || "Uncategorized"; // Default if category is missing
    ...
    console.log("Category-wise expenses:", categoryExpenses);
    ...
    return res.json({
      success: true,
      monthlyCategoryExpenses: formattedExpenses
    });
